...
export const useFindCar = () => {
    ...
    const fetchActiveData = useCallback(async () => {
        ...
        setLoading(true);
        try {
            socket.emit('getPage', filters, (response: WebSocketResponse<ParkingActiveResponse>) => {
                if (response.success) {
    ...
    const searchByAdmin = useCallback(async (searchFilters: ProcessedFilters) => {
        if (!socket?.connected || !isConnected) {
            setError('รอการเชื่อมต่อ...');
            return;
        }
        ...
        setLoading(true);
        try {
            socket.emit('searchAdmin', searchFilters, (response: WebSocketResponse<ParkingActiveResponse>) => {
                if (response.success) {
                    setActiveData(response.data);
                    setError(null);
                } else {
    ...
    const handleSearch = useCallback((searchParams: SearchFields) => {
        ...
        // อัปเดตสถานะการค้นหา
        setIsSearchMode(hasSearchValues);
    
        // ประมวลผลค่าและแปลงเป็นรูปแบบที่ต้องการ (เช่น แปลงวันที่เป็น ISO string)
        const processedParams = Object.entries(paramsToCheck).reduce((acc, [key, value]) => {
            if (value === undefined) {
                acc[key] = null;
        ...
        // เรียกใช้ฟังก์ชันค้นหาหรือดึงข้อมูลตามสถานะการค้นหา
        if (hasSearchValues) {
            searchByAdmin(updatedFilters);
        } else {
            fetchActiveData();
        }
    }, [filters, searchByAdmin, fetchActiveData]);
    ...
    const fetchSlotData = async (slotID: string) => {
        ...
        setLoading(true);
        try {
            const response = await fetch(`${API_CONFIG.baseURL}/parkingslot/${slotID}`, {
                method: 'GET',
                headers: {
    ...
    const handleClick = useCallback((car: CarData) => {
        setSelectedCar(car);
        console.log(car);
    ...
    const handleActiveUpdate = useCallback((data: any) => {
